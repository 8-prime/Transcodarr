...
{
    private readonly Dictionary<string, int> _defaultEncoderSlots =
        new() { { "libx265", 1 }, { "hevc_nvenc", 2 }, { "hevc_amf", 1 }, { "hevc_qsv", 1 } };
    ...
    private async Task<bool> IsEncoderAvailableAsync(string encoderName)
    {
        ...
        try
        {
            return await FFMpegArguments
                .FromFileInput("color=c=black:s=1920x1080:r=1:d=1", false, o => o.ForceFormat("lavfi"))
                .OutputToFile(nullSink, true, o => o
        ...
        {
            logger.LogError(ex, "An error occured during FFMpeg processing of encoder check for {EncoderName}",
                encoderName);
            return false;
        }
    }
