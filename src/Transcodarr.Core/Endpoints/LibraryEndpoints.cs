...
public static class LibraryEndpoints
{
    ...
    private static async Task<Ok<List<LibraryResponse>>> GetLibraries([FromServices] TranscodarrDbContext dbContext,
        CancellationToken stoppingToken)
    {
        var libraries = await dbContext.Libraries.Select(l => new LibraryResponse
        {
            Id = l.Id,
    ...
    private static async Task<Results<Ok, BadRequest<string>>> AddLibrary([FromBody] CreateLibraryRequest request,
        [FromServices] ConfigurationService configurationService, [FromServices] TranscodarrDbContext dbContext,
        CancellationToken stoppingToken)
    {
        if (!configurationService.Initialized)
        {
            return TypedResults.BadRequest("Settings must be initialized before libraries can be added");
        }
    ...
    private static async Task<Results<Ok, NotFound>> DeleteLibrary(Guid id,
        [FromServices] TranscodarrDbContext dbContext, CancellationToken stoppingToken)
    {
        return await dbContext.Libraries.Where(l => l.Id == id).ExecuteDeleteAsync(stoppingToken) == 0
            ? TypedResults.Ok()
